  ...
  class << self
    ...
    def expect(passed = nil, message = nil, options = {}, &block)
      ...
      if block_given? ? block.call() : !!passed
        ...
        log '<div class="console-passed" style="color: green;">' + success_msg + '</div>', true
      else
        message ||= "Something is wrong"
        log "<div class='console-failed' style='color: red;'>Test Failed: " + message.to_s + "</div>", true
...
File.open('results.html', 'w+') do |file|
  Dir.glob('./src/**/*.rb').sort.each do |path|
    puts path
    results = capture_stdout { eval(File.read(path)) }
    file.write("<h2>#{path}</h2>")
    file.write(results)
    file.write("<br><br>")
  end
end
